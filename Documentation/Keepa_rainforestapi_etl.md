...
Loaded **Transformed**, **uncleaned** data, extracted from **Keepa** and **RainforestAPI** sources. 

## 2. Silver Layer (Data Cleaning)

### 1. **What are usual cleaning stages:**

1. Replace Null values
2. Go about duplicated rows
3. Data types
4. Normalizing Boolean values
5. Extra cleaning on the data

**What are the extra Cleaning Data:**

As we ran api scripts to rainforest api on ASINs we extcracted from Keepa API, if those ASINs arew not stored in rainforest endpoint, rainforest will automatically extract another ASINs that might not be in our historical data therefore we need to clean our keepa and rainforest data in 2 ways:

1. Only keep rows from rainforest datasets Where asins are in keepa asins (product or offer asins in keepa's static)

2. Only keep rows from keepa datasets Where asins are in rainforest product asins (historical in product asins)

3. Example script on 1 and 2:
...
4. If we have duplicated rows due to multiple asin extraction within keepa api, only keep the rows from product that seller, product asin is same in offer table and that offer is buy box!

   1. SEE FULL EXAMPLE BELLOW

**NOTE:**  Performing 1 and 2 in extra cleaning will halve the products from ~1900 to ~950 product records.
...
2. `offers`:  As there are nor Amazon `Seller_id` in Rainforest api endpoint all `Seller_id` for `amazon.com` and `amazon retail` seller is null. 

   1. We will fill it up using conditional formatiing using Keepa API 
   2. Or custome ID if not existed
